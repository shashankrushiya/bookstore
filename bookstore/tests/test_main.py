...
async def test_create_user_signup_email_exists(mock_db, mock_pwd_context):
    mock_db.query().filter().first.return_value = {"email": "test@example.com", "password": "password"}
    user_credentials = {"email": "test@example.com", "password": "password"}
...
async def test_login_success(mock_db, mock_pwd_context, mock_create_access_token):
    mock_db.query().filter().first.return_value = {"email": "test@example.com", "password": "hashed_password"}
    user_credentials = {"email": "test@example.com", "password": "password"}
